...
class ProcedureAnalyzer:
    ...
    def _extract_cursors(self, code: str) -> List[str]:
        """커서 선언 추출"""
        cursors = []
        pattern = r'\bCURSOR\s+([A-Z_][A-Z0-9_]*)'
    ...
    def _extract_tables(self, statement: str, stmt_type: str) -> List[str]:
        ...
        for pattern_name, pattern in self.TABLE_PATTERNS.items():
            ...
            for match in matches:
                ...
                # 스키마명 제거 (RWIS.TABLE_NAME -> TABLE_NAME)
                if '.' in table_name:
                    table_name = table_name.split('.')[-1]
                
                # 별칭이나 예약어 제외
                if table_name.upper() not in ['DUAL', 'X', 'Y', 'A', 'B', 'T']:
...
def main():
    ...
    # 분석기 초기화
    analyzer = ProcedureAnalyzer('/mnt/user-data/uploads/records.json')
    ...
    # 레포트 생성
    print("📊 HTML 레포트 생성 중...")
    output_path = '/mnt/user-data/outputs/procedure_analysis_report.html'
